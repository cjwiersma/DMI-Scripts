...
# Function to get logon events from remote computer and export to Excel
function Get-RemoteLogonEvents {
    ...
    try {
        $ScriptBlock = {
            ...
            # Retrieve events for the last 3 days to make sure we're not missing any (larger window)
...
# Monitor for logon events and export them to Excel
Write-Host "Starting logon event monitoring. Press Ctrl+C to stop."
while ($true) {
    Write-Host "`nChecking for logon events from the last 8 hours..."
    Get-RemoteLogonEvents -ComputerName $ComputerName -Hours 8
    Start-Sleep -Seconds 3600  # Sleep for 1 hour before checking again
}
