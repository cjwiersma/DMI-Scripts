# Define the path to the group members file
$groupMembersPath = "\\sys22\vol1\SysShare\Utilities\groupmembers.txt"

# Read the usernames from the group members file
$users = Get-Content -Path $groupMembersPath
$serverPaths = @(
    "\\DMI4\HOME\",
    "\\DMI9\HOME\",
    "\\DMI11\HOME\"
...
foreach ($user in $users) {
	$adu = Get-ADUser -Identity $user -Properties SamAccountName, Name
    $samAccountNames = $adu.SamAccountName
    $displayNames = $adu.Name
